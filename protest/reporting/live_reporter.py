"""Live reporter with real-time display for parallel test execution."""
...
class LiveReporter(PluginBase):
    """Live reporter with spinners and real-time updates."""
    ...
    def _is_running(self, phase: TestPhase) -> bool:
        return phase in (TestPhase.SETUP, TestPhase.RUNNING, TestPhase.TEARDOWN)
    ...
    def on_suite_teardown_start(self, name: str) -> None:
        self._suite_teardowns[name] = SuiteTeardown(
            name=name, start_time=time.perf_counter()
        )
        self._refresh()
