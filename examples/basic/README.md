...
# Run async demo
uv run protest run demo_async:session

# Run capture demo (stdout/stderr capture)
uv run protest run demo_capture:session

# Run factory demo
uv run protest run factory_demo:session

# Run caplog demo (log capture)
uv run protest run caplog_demo:session
```
...
uv run protest run demo:session -n 4

# Run 8 tests concurrently
uv run protest run demo_async:session -n 8
```
...
uv run protest run demo:session --lf

# Combine with concurrency
uv run protest run demo:session --lf -n 4
```
...
uv run protest run demo:session --cache-clear

# Clear cache + concurrency
uv run protest run demo:session --cache-clear -n 4
```
...
```bash
# List all tests
uv run protest run demo:session --collect-only

# Combine with --lf to see which tests would run
uv run protest run demo:session --lf --collect-only
```
...
uv run protest run demo:session --cache-clear --lf

# Typical workflow: first run all, then iterate on failures
uv run protest run demo:session -n 4           # Run all
uv run protest run demo:session --lf           # Fix failures, re-run only those
uv run protest run demo:session --lf           # Keep iterating
uv run protest run demo:session -n 4           # Final full run
```
